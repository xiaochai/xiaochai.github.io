...
在对象的方法内，我们应该只调用属于以下类型的方法：此对象本身的、被当成方法的参数而传进来的对象的、此方法所创建或者实例化的任何对象的、对象的任何组件的。
...
Java的I/O库使用了装饰者模式，例如输入类的抽象基类为InputStream，有一些具体组件FileInputStream、StringBufferInputStream可以LineNumberInputStream、DataInputStream这些类装饰起来。
...
简单工厂：并非严格意义上的设计模式，而更多的是一种编程习惯，将条件判断创建不同类的代码提取独立出来以供复用。
...
队列：往队列中添加命令，而消费队列的进行将命令取出调用其execute方法，而不需要关心此命令的细节，达到队列与处理对象之间的解耦。
...
简单整合两个菜单的话，遍历的代码变成如下，如果多个地方都写这一长串的代码，会很不久好。更重要的是，如果又合并一家餐厅，那么我们就要对所有遍历的地方做改动，那是个灾难。
...
餐厅又收购了C餐厅，这个餐厅的麻烦点是他的菜单不紧有主菜单，而且还有子菜单。现在的菜单结构图看起来像是一个树型结构。

所以为了更好的扩展，我们使用组合模式重新实现菜单类。

树的结点分成叶子节点(MenuItem)和非叶子结点(Menu)，所以我们创建两个类，他们都继承自同一个超类(MenuComponent)。
...
声明了一个状态机类了继承自状态基类的4个状态类，对应图中的4个状态，然后状态机收到的行为会转到当前状态里，由当前状态对象来执行对应方法。
...
写时复制代理：用户控制对象的得势，方法是延迟对象的复制，直到真正需要。
...
对于操作系统和软件，例如有OS1和OS2两个操作系统，又有Software1和Software2两个软件运行这两个操作系统。
...
可以让对象之间所wfuxr消息变得简单而大幅度减少

### 缺点

如果设计不当，中介者本身会变得过于复杂。

如果同分布系统增加中心节点可以简化调度一样，中心节点也会成为瓶颈。
...
现实某些情况可使用序列化来代理此模式的实现（序列化可能也算是备忘录的实现）。
